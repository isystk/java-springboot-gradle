...
public class PostService extends BaseTransactionalService {
	...
	public Optional<FrontPostDto> findMyDataById(Integer postId) {
		...
		if (!post.getUserId().equals(userHelper.getLoginUser().getUserId())) {
			throw new NoDataFoundException("データが見つかりません。post_id=" + postId + " user_id=" + userHelper.getLoginUser().getUserId());
		}
	...
	private FrontPostDto convertTPostToFrontPostDto(TPostRepositoryDto tPostRepositoryDto) {
		...
		if (tPostRepositoryDto.getTPostImageList() != null) {
			for (TPostImage tPostImage : tPostRepositoryDto.getTPostImageList()) {
				...
				imageDto.setImageUrl(imageHelper.getUrl(tPostImage.getImageId(), ImageSuffix.SQUARE.getSuffix()));
				imageList.add(imageDto);
			}
		}
		dto.setImageList(imageList);

		dto.setRegistTimeYYYYMMDD(DateUtils.format(tPostRepositoryDto.getRegistTime(), DateTimeFormatter.ofPattern("yyyy/MM/dd")));
		dto.setRegistTimeMMDD(DateUtils.format(tPostRepositoryDto.getRegistTime(), DateTimeFormatter.ofPattern("MM/dd")));
		return dto;
	}
